...
class Learner(object):
  ...
  def run_epochs(self, model, train_data, val_data):
    ...
    for epoch in range(self.epochs):
      val_every = self.monitor.start_epoch(self.debug, self.logger)
      ...
      for batch in train_data.batch(self.batch_size, shuffle=True):
        ...
        if self.monitor.time_to_validate():
          ...
          if self.monitor.best_so_far():
            ...
            self.module.save_checkpoint(self.monitor)
            self.module.prune_saves()
            val_data.record_preds(preds=self.module.run_glad_inference(val_data),
