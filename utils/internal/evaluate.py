...
if __name__ == "__main__":
  ...
  print val_variables.shape
  # ---- BUILD MODEL ----
  encoder = torch.load('filename.pt')
  decoder = torch.load('')
  # ------ EVALUATE ------
  encoder_hidden = encoder.initHidden()
  ...
  for di in range(target_length):
    ...
    _, max_index = decoder_output.max(1)
    predicted_indices.append(nimax_index)
    # decoder_output = predicted output distribution (1x2165) Float Tensor
    # target_variable[di] = correct word? Long Tensor (scalar 1)
  ...
  pred_variable = torch.cat(predicted_indices)
  print "Predicted --------"
  print pred_variable
  print "Target -----------"
