...
from objects.models.attention import Attention, DoubleDotAttention
...
class EntNetEntitySelector(nn.Module):
  """
  Select entities using hidden state from encoder, gating mechanism
  ...
  def __init__(self, config):
    super().__init__()
    # self.attend = Attention(act="sigmoid")

    # Initialize attention function over entities
    self.double_attend = DoubleDotAttention(act="sigmoid")
    # Initialize attender to entities
    self.single_attend = Attention()

    self.config = config

  def forward(self, hidden, entities, keys):
    # Compute attention weights over entities
    dist, act = self.double_attend(keys, entities, hidden)
    # Attend to the entities
    selected_entities = self.single_attend(entities, dist)

    return selected_entities, dist, act

  def cuda(self, device_id):
    super(EntNetEntitySelector, self).cuda(device_id)
    self.double_attend.cuda(device_id)
    self.single_attend.cuda(device_id)
    self.is_cuda = True
