...
from torch import load, save, manual_seed
...
from model.learn import Builder, Learner
...
def validate(input_variable, target_variable, encoder, decoder, criterion, task, kind):
...
def track_progress(args, model, verbose, debug, processor, builder, task,
            kind, epochs):
  ...
  enc_optimizer, dec_optimizer = builder.init_optimizers(encoder, decoder)
      ...
      if iteration > 0 and iteration % val_every == 0:
        ...
        for val_input, val_output in processor.val_data:
          val_loss, bleu_score, turn_success = validate(val_input, \
                val_output, encoder, decoder, criterion, task, kind)
...
if __name__ == "__main__":
  ...
  model = builder.build_model(vocab.ulary_size(task), vocab.label_size(kind))
  results = track_progress(args, model, args.verbose, args.debug, processor,
       builder, task, kind, args.epochs)
  # ------- MANAGE RESULTS -------
  if not results[0].completed_training: sys.exit()
  if args.save_model:
